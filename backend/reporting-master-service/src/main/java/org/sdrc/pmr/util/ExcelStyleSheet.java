...
public class ExcelStyleSheet {
	 ...
	 * @return
	 */
	public static SXSSFSheet doMerge(int rowIndex, int columnIndex, int rowSpan, int columnSpan, SXSSFSheet sheet) {
...
// odd cell
public static CellStyle getStyleForHeader(Workbook workbook, Boolean lockCheck) {
	CellStyle colStyleHeader= workbook.createCellStyle();
	colStyleHeader.setBorderBottom(BorderStyle.THIN);
	colStyleHeader.setBorderTop(BorderStyle.THIN);
	colStyleHeader.setBorderLeft(BorderStyle.THIN);
	colStyleHeader.setBorderRight(BorderStyle.THIN);
	colStyleHeader.setAlignment(HorizontalAlignment.CENTER);
	//colStyleHeader.setShrinkToFit(true);
	colStyleHeader.setWrapText(false);
	colStyleHeader.setFillForegroundColor(IndexedColors.LIGHT_ORANGE.getIndex());
	colStyleHeader.setFillPattern(FillPatternType.SOLID_FOREGROUND);
	colStyleHeader.setLocked(false);

	if (lockCheck) {
		colStyleHeader.setLocked(true);
	}
	return colStyleHeader;
}

public static CellStyle getStyleForSkill(SXSSFWorkbook workbook, boolean lockCheck) {
	CellStyle colStyleHeader= workbook.createCellStyle();
	colStyleHeader.setBorderBottom(BorderStyle.THIN);
	colStyleHeader.setBorderTop(BorderStyle.THIN);
	colStyleHeader.setBorderLeft(BorderStyle.THIN);
	colStyleHeader.setBorderRight(BorderStyle.THIN);
	colStyleHeader.setAlignment(HorizontalAlignment.LEFT);
	//colStyleHeader.setShrinkToFit(true);
	colStyleHeader.setWrapText(false);
	colStyleHeader.setFillForegroundColor(IndexedColors.GOLD.getIndex());
	colStyleHeader.setFillPattern(FillPatternType.SOLID_FOREGROUND);
	colStyleHeader.setLocked(false);
